...
def generate_internal_links_with_embeddings(article_content: str, pages_with_embeddings: List[Dict], 
    ...
    try:
        ...
        # Generate internal links with a clearer, more encouraging prompt
        response = openai.ChatCompletion.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are an SEO expert specializing in internal linking. Your task is to identify relevant phrases in content and convert them into hyperlinks."},
                {"role": "user", "content": f"""
                Insert {max_links} internal links into this HTML article by replacing relevant existing text with linked versions.

                Example of good linking:
                Original: "Victorian style windows are popular for historic homes."
                With link: "Victorian style <a href="https://example.com/windows">windows are popular</a> for historic homes."

                Instructions:
                1. Find phrases in the existing content that match topics in the site pages listed below
                2. REPLACE those phrases with <a href="URL">same text</a> format
                3. Choose 2-6 word phrases from the EXISTING text to use as anchor text
                4. Use each URL only ONCE
                5. DO NOT add any new text - only convert existing text to links
                6. Keep all HTML tags (h1, h2, h3, p, ul, li) intact
                7. Try to spread links throughout the article
                8. You MUST add at least {max_links//2} links
                ...
                Return:
                ARTICLE_WITH_LINKS:
                [article content with links inserted]
                
                LINKS_ADDED:
                [
                    {{
                        "url": "the URL you linked to",
                        "anchor_text": "the exact text you used as anchor",
                        "context": "short phrase showing ...anchor text... in context"
                    }}
                ]
                """}
            ],
            temperature=0.5  # Increased temperature to encourage creativity
        )
        
        result = response.choices[0].message.content
        logger.info("Received response from OpenAI for internal linking")
        ...
        if not article_match:
            logger.warning("Could not find 'ARTICLE_WITH_LINKS:' pattern in response")
        ...
        if article_content == article_with_links:
            logger.warning("No changes detected between original article and article with links")
        ...
        if links_match:
            ...
            if json_match:
                try:
                    ...
                    logger.info(f"Successfully extracted {len(links_added)} links from response")
                except Exception as e:
                    logger.error(f"Failed to parse links JSON: {e}")
                    links_added = []
            else:
                logger.warning("Could not find valid JSON array in links section")
        ...
        if links_added:
            ...
            logger.info(f"Final count after removing duplicates: {len(links_added)} links")
